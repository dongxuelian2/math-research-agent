...
export class ProofRuntime {
	...
	private async loadPersistedState(): Promise<void> {
		try {
			...
			// Session events are appended before state.json is replaced. If a process
			// exits in that narrow window, the append-only log is newer than this
			// snapshot. Replay it after loading the snapshot so a durable Worker or
			// Verifier receipt cannot be forgotten and executed a second time.
			this.restoreFromSession();
