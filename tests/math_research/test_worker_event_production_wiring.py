...
from math_research_agent.research.research_policy import ResearchPolicy
...
def test_t13_production_planner_worker_verifier_truth_mutation_e2e(tmp_path: Path):
    ...
    events = sorted(run_dir.glob("engine/steps/*/workers/event_*.json"))
    verifier_events = sorted(run_dir.glob("engine/steps/*/workers/verifier_event_*.json"))
    assert len(events) >= 3
