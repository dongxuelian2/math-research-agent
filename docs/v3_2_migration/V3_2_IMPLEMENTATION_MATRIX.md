...
| Canonical project artifact authority | body+hash+provenance or scoped block | partial/no proof-replay resolver | typed requirement/resolver, body-bound provenance, scoped blockers, resume revalidation, promotion prerequisite | IMPLEMENTED | `ReplayPolicy→CanonicalArtifactResolver→Orchestrator/ContextBuilder→AsyncDAGScheduler→promotion guard` | A–H + production manifest/orchestrator tests | checkpoint digest or immutable content-addressed body | Git-object recovery is intentionally not implemented |
