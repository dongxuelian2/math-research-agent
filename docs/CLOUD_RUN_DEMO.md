...
`2`), and prints the deployed URL plus a live `/healthz` response.

## Video/demo proof

1. Keep the successful `gcloud run deploy` output visible and open the printed
   `https://...run.app/healthz` URL in a browser.
...
Cloud Run revision fields from `/healthz`, and the real backend API response is
