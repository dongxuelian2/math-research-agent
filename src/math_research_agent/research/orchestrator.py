"""Outer orchestration and audit gate around Math Research Agent's core Prover."""
...
class ResearchOrchestrator:
    """Drive a durable, resumable project run around the local research engine."""
    ...
    def run(self, *, stop_after: str | None = None) -> dict:
        ...
        if not self._phase_at_least("CANDIDATE_READY"):
            try:
                self._run_math_research_agent_candidate()
            ...
            if not candidate_path.exists():
                ...
                self.target = self._transition_truth(
                    "PARTIAL",
                    actor="MasterPlanner",
                    reason="Math Research Agent ended without a candidate proof",
                    audit_status="NOT_AUDITED",
                )
                self._checkpoint(
                    "COMPLETE",
                    status="PARTIAL",
                    failure_reasons=["Math Research Agent ended without PROOF.md"],
            ...
            self.target = self._transition_truth(
                "CANDIDATE_PROOF",
                actor="MasterPlanner",
                reason=f"Math Research Agent candidate produced in {self.run_dir.name}; not yet audited",
...
"""
    ...
    def _close_research_session(
        ...
        for path in sorted((self.run_dir / "engine" / "steps").glob("**/workers/*")):
            if path.is_file():
        ...
        verifier_paths = sorted(
            (self.run_dir / "engine" / "steps").glob("**/workers/verifier_event_*.json")
        )
