...
from math_research_agent.providers.support import archive
...
def _command_from_candidate(
    path: Path, source: str, environment: dict[str, str]
) -> CodexCLICommand | None:
...
def _npm_global_candidates(environment: dict[str, str]) -> list[Path]:
    prefixes: list[Path] = []
    configured_prefix = environment.get("npm_config_prefix") or environment.get("NPM_CONFIG_PREFIX")
...
def serialize_codex_prompt(
    ...
    *,
    ...
    if tools:
        envelope["available_math_research_agent_tools"] = tools
    return (
...
def _classify_failure(message: str, returncode: int | None) -> tuple[str, bool, str]:
    ...
    return "process_failed", False, (f"Codex CLI exited unsuccessfully (exit code {returncode}).")
...
def _parse_jsonl(stdout: str) -> dict[str, Any]:
    ...
    for event in events:
        ...
        if event.get("type") == "turn.completed" and isinstance(event.get("usage"), dict):
            ...
            usage = {
                ...
                "reasoning_tokens": int(
                    reported.get("reasoning_output_tokens", reported.get("reasoning_tokens", 0))
                    or 0
                ),
                "cached_tokens": int(
                    reported.get("cached_input_tokens", reported.get("cached_tokens", 0)) or 0
                ),
                "total_tokens": int(
        ...
        if event.get("type") in {"turn.failed", "error"}:
            ...
            event_error = (
                json.dumps(error, ensure_ascii=False) if not isinstance(error, str) else error
            )
    return {
...
class CodexCLIClient:
    """Math Research Agent-compatible client backed by one `codex exec` per request."""
        ...
        *,
        ...
        if self._interrupted.is_set():
            raise self._error(
                ...
                retry_count=0,
                retryable=False,
                explanation="Codex CLI call was cancelled before start.",
            )
        ...
        while True:
            attempt_dir = (
                self.working_dir / f"call-{call_num:03d}" / f"attempt-{retry_count + 1:02d}"
            )
            ...
            try:
                ...
                error = self._error(
                    error_type="unknown_codex_error",
                    status=None,
                    retry_count=retry_count,
                    retryable=False,
            ...
            if "result_text" in locals() and not isinstance(
                locals().get("error"), CodexCLIProviderError
            ):
                break
