...
from math_research_agent.core import Budget, ResearchEngine
...
class CandidateEngine(_OwnerComponent):
    """Connect the research layer to the project-owned candidate engine."""
    ...
    def run(self) -> None:
        ...
        engine_dir = self.run_dir / "engine"
        ...
        resumed = (engine_dir / "WHITEBOARD.md").exists() and not (engine_dir / "PROOF.md").exists()
        ...
        policy = ResearchPolicy(**policy_kwargs)
        ...
        # candidate generation, but the proof task is owned by our local engine.
        self._start_async_pipeline_monitor(include_proof=False)
        planner = make_planner(archive / "planner")
        worker = make_worker(archive / "worker")
        ...
        try:
            proof_path = engine.run()
        finally:
            for client in holders.values():
                client.cleanup()
            self._stop_async_pipeline_monitor()
        candidate_exists = proof_path is not None and proof_path.exists()
        if candidate_exists:
            proof_path.replace(self.run_dir / "CANDIDATE_PROOF.md")
            proof_path = self.run_dir / "CANDIDATE_PROOF.md"
        if active_proof_task:
            self.pipeline_scheduler.complete_task(
                active_proof_task,
                {
                    "success": candidate_exists,
                    "proof_candidate": candidate_exists,
                    "high_value": candidate_exists,
                },
            )
        ...
        workers = len(list(engine_dir.glob("steps/*/workers/worker_*_call.md")))
        verifiers = len(list(engine_dir.glob("steps/*/workers/verifier_*_call.md")))
        self.metrics["worker_and_verifier"] = {
