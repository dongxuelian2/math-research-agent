...
logger = logging.getLogger("math_research_agent.lean")
...
def run_lean_check(
    ...
    if _os.name == "nt":
        ...
        group_kwargs = {"preexec_fn": _lean_preexec(max_memory_mb)}
    try:
...
class LeanWorkDir:
    """Manages the Math Research Agent-{id} subdirectory within a Lean project."""

    def __init__(self, project_dir: Path):
        self.project_dir = project_dir
        self.random_id = secrets.token_hex(4)  # 8 hex chars
        self.dir = project_dir / f"Math Research Agent-{self.random_id}"
