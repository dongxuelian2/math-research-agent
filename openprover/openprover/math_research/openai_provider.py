...
class OpenAIProviderError(RuntimeError):
    ...
    def __init__(self, *, error_type: str, status: int | None, role: str,
                 model: str, retry_count: int, retryable: bool,
...
class OpenAIResponsesClient:
    ...
    def __init__(self, model: str, archive_dir: Path, *, api_key: str,
                 role_name: str, reasoning_effort: str | None = None,
                 timeout_seconds: float = 600.0, max_retries: int = 2,
    ...
    def _execute(self, *, input_items: list[dict], prompt_for_archive: str,
            ...
            try:
                ...
                if info["retryable"] and retry_count < self.max_retries:
                    ...
                    logger.warning(
                        ...
                        label, retry_count, self.max_retries, delay,
                    )
                    if delay:
                        self._sleep(delay)
                    continue
                ...
                self._archive(
                    call_num, label, prompt_for_archive, system_prompt,
                    json_schema, None, provider_error, elapsed_ms, archive_path,
                )
        ...
        if getattr(response, "status", "") == "failed":
            ...
            provider_error = self._provider_error(
                exc, retry_count=retry_count, retry_exhausted=False,
                error_type=getattr(error, "code", None) or "response_failed",
            )
            self._archive(
                call_num, label, prompt_for_archive, system_prompt,
                json_schema, _raw_response(response), provider_error,
    ...
    def _classify_error(self, exc: Exception) -> dict[str, Any]:
        ...
        quota = code in {"insufficient_quota", "billing_hard_limit_reached"} or any(
            phrase in lowered for phrase in (
                "insufficient_quota", "billing hard limit", "quota exceeded",
            )
        )
        retryable = False
    ...
    def _provider_error(self, exc: Exception, *, retry_count: int,
                        retry_exhausted: bool,
                        error_type: str | None = None) -> OpenAIProviderError:
        info = self._classify_error(exc)
        kind = error_type or type(exc).__name__
        if info["quota"]:
            kind = "quota_exceeded"
            explanation = "OpenAI API quota or billing limit rejected the request; no retry was attempted."
            ...
            explanation = "A transient OpenAI API failure persisted after the configured bounded retries."
        else:
    ...
    def _archive(self, call_num, label, prompt, system_prompt, json_schema,
        ...
        archive(
            self.model, self.archive_dir, call_num, label, prompt,
