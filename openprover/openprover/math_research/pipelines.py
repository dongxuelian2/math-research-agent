...
class AtomicResourceBudget:
    """Thread-safe campaign-level reservation gate.

    Pipeline tasks may be created independently, but provider calls reserve
    against one shared hard budget before a worker starts.  The reservation is
    deliberately small (one call plus optional token estimates) so callers can
    commit actual usage after the provider returns without racing another
    pipeline.
    """
    ...
    @property
    def usage(self) -> dict:
        """Backward-compatible committed usage view."""
...
def migrate_pipeline_state(raw: dict | None) -> dict:
    """Migrate old/missing checkpoint fields without replaying completed work."""

    value = copy.deepcopy(raw) if isinstance(raw, dict) else {}
    version = int(value.get("schema_version", 1))
    if version > 3:
        raise ProjectError(f"Unsupported pipeline state schema: {version}")
    # Keep the public schema number at v2 for old checkpoint compatibility;
    # v3 fields are additive and migrate deterministically.
    ...
    literature_defaults = {
        ...
        "sources_found": 0,
        "sources_deep_read": 0,
        "external_theorems_extracted": 0,
        "verified_external_authorities": 0,
        "exact_matches": 0,
        "partial_matches": 0,
        "method_matches": 0,
        "literature_guided_closures": 0,
        "duplicate_searches_avoided": 0,
        "proof_calls_avoided_due_to_literature": 0,
    }
    value.setdefault("literature", {})
    for key, default in literature_defaults.items():
        value["literature"].setdefault(key, default)
    # Clean queue references left by a crash after an atomic task completion.
...
def applicability_assumption_snapshot(
    ...
    """Stable identity for the project-truth inputs to applicability."""
...
class AsyncDAGScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        self.state = migrate_pipeline_state(state)
    ...
    def plan_literature_searchers(
        ...
        obligation_id: str,
        *,
        search_tasks: list[dict] | None = None,
        strategies: list[str] | None = None,
        lead_task_id: str | None = None,
    ) -> list[dict]:
        # ``strategies`` remains a migration-only input for old checkpoints.
        # Production Lead results must provide complete structured search_tasks.
        if search_tasks is None and strategies is not None:
            obligation = self._obligation(obligation_id)
            hints = (obligation.get("literature_request") or {}).get("search_hints")
            query = ""
            if isinstance(hints, str):
                query = hints
            elif isinstance(hints, list):
                query = " ".join(str(item) for item in hints if str(item).strip())
            search_tasks = [
                {"strategy": strategy,
                 "public_query": query or f"public mathematics {str(strategy).replace('_', ' ')}",
                 "reason": "legacy checkpoint migration", "priority": "MEDIUM"}
    ...
    def create_citation_chain_task(
        ...
        return self.create_task(
            ...
            payload={"search_strategy": "citation_chain", "source_id": source_id},
        ...
        *,
        ...
        with self._lock:
            ...
            if verdict in {"EXACT_RESULT_FOUND", "STRONGER_RESULT_FOUND"}:
                ...
                if authority_status in {"VERIFIED_SOURCE_THEOREM", "VERIFIED_EXTERNAL_AUTHORITY"}:
    ...
    def reuse_verified_source_theorem(
        ...
        if authority_record.get("status") not in {
    ...
    def _complete_literature(self, task: dict, result: dict, obligation: dict) -> None:
        ...
        if role == "literature_lead":
            ...
            if search_tasks is not None or strategies is not None:
                self.plan_literature_searchers(
                    task["obligation_id"],
                    search_tasks=list(search_tasks) if search_tasks is not None else None,
            ...
            verified = result.get("authority_status") in {
                "VERIFIED_SOURCE_THEOREM", "VERIFIED_EXTERNAL_AUTHORITY",
            }
            # Direct scheduler callers from the legacy unit API remain
            # compatible; production runtime results must carry the registry's
            # deterministic marker.
            if verified and (result.get("deterministic_verification") or self.runtime is None):
    ...
    def _complete_verification(self, task: dict, result: dict, obligation: dict) -> None:
        ...
        if task.get("role") == "theorem_verifier" and task.get("payload", {}).get(
            ...
            return
        if task.get("role") == "reconstruction" and obligation.get("authority_status") in {
            "VERIFIED_SOURCE_THEOREM", "VERIFIED_EXTERNAL_AUTHORITY",
        }:
            verdict = "MISSING_APPLICABILITY_RECONSTRUCTION"
        if obligation.get("authority_status") in {
            "VERIFIED_SOURCE_THEOREM", "VERIFIED_EXTERNAL_AUTHORITY",
            ...
            for attempt in range(6):
                try:
                    ...
                    if attempt == 5:
                        ...
                        raise
                    # Windows can transiently retain a reader/replace handle.
