...
class Prover:
    _SCOPE_CLOSURE_MARKERS = (
        "SCOPE_CLOSURE: PASS",
        "H_SCOPE_BRIDGE: PASS",
        "H-SCOPE BRIDGE: PASS",
        "SCOPE BRIDGE: PASS",
    )
    _SCOPE_BLOCKER_PATTERNS = (
        re.compile(r"\bSCOPE[_ -]?GAP\b", re.IGNORECASE),
        re.compile(r"\bUNRESOLVED[_ -]?SCOPE\b", re.IGNORECASE),
        re.compile(
            r"\bREQUIRED[_ -]?DEPENDENCY[_ -]?EXPANSION\b",
            re.IGNORECASE,
        ),
        re.compile(
            r"\bEXTERNAL DEPENDENCY EXPANSION REQUIRED\b",
            re.IGNORECASE,
        ),
        re.compile(
            r"\b(?:SCOPE|H[- ]?SCOPE).{0,160}\bBLOCKED\b",
        ...
        re.compile(
            r"\b(?:MUST NOT|DO NOT)\s+SUBMIT\b.{0,160}\b(?:INCOMPLETE|SCOPE|GAP|BLOCKED)\b",
            re.IGNORECASE | re.DOTALL,
        ),
    )

    @classmethod
    def _scope_submission_blocker(cls, whiteboard: str) -> str | None:
        """Return an unresolved scope blocker before accepting a proof.

        A planner may keep an earlier failed attempt on the whiteboard, but an
        explicit closure marker records that the gap was subsequently closed.
        Without that marker, explicit scope/dependency blockers must prevent a
        candidate from being submitted to the outer audit gate.
        """
        if not whiteboard:
            return None
        upper = whiteboard.upper()
        if any(marker in upper for marker in cls._SCOPE_CLOSURE_MARKERS):
            return None
        for pattern in cls._SCOPE_BLOCKER_PATTERNS:
    ...
    def _save_step_history(self):
        ...
        path.write_text(json.dumps(self.step_history, ensure_ascii=False), encoding="utf-8")
    ...
    def _load_step_history(self):
        ...
        if path.exists():
            try:
                self.step_history = json.loads(path.read_text(encoding="utf-8"))
    ...
    def _maybe_respawn_interrupted_workers(self):
        ...
        if not toml_file.exists():
            return
        plan = prompts.parse_saved_step_toml(toml_file.read_text(encoding="utf-8"))
    ...
    def _handle_submit_proof(self, plan: dict, _step_dir: Path) -> str:
        ...
        scope_blocker = self._scope_submission_blocker(self.whiteboard)
        if scope_blocker:
            self.tui.log(f"submit_proof blocked: {scope_blocker}", color="red")
            self._push_output(
                "submit_proof rejected: "
                f"{scope_blocker}. Resolve the gap and record "
                "SCOPE_CLOSURE: PASS on the whiteboard before submitting."
            )
            return "continue"

        self.proof_text = content
        (self.work_dir / "PROOF.md").write_text(content, encoding="utf-8")
    ...
    def _handle_submit_lean_proof(self, plan: dict, step_dir: Path) -> str:
        ...
        if success:
            ...
            (self.work_dir / "PROOF.lean").write_text(proof_text, encoding="utf-8")
    ...
    def _handle_read_theorem(self):
        ...
        if proof_lean_path.exists():
            parts.append(
                f"\n\n## PROOF.lean\n\n```lean\n{proof_lean_path.read_text(encoding='utf-8')}\n```"
            )
        self._push_output("\n".join(parts))
        ...
        if n:
            with ThreadPoolExecutor(max_workers=n) as pool:
                ...
                for i, task in enumerate(tasks):
                    # Save task
                    desc = task.get("description", "")
                    (workers_dir / f"task_{i}.md").write_text(desc, encoding="utf-8")
    ...
    def _handle_literature_search(self, plan: dict, step_dir: Path,
        ...
        while True:
            ...
            try:
                ...
                (workers_dir / "result_0.md").write_text(result, encoding="utf-8")
                break
            except Interrupted:
                self.tui.stream_end(tab=wid)
                self.worker_llm.clear_interrupt()
        ...
        for idx, step_dir in enumerate(step_dirs):
            ...
            if action == "read_theorem":
                ...
                if proof_lean.exists():
                    parts.append(
                        f"\n\n## PROOF.lean\n\n```lean\n{proof_lean.read_text(encoding='utf-8')}\n```"
                    )
            ...
            for task_file in task_files:
                ...
                desc = task_file.read_text(encoding="utf-8")
                result = result_file.read_text(encoding="utf-8") if result_file.exists() else ""
