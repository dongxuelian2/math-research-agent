...
def _verified_source(tmp_path, authority_id="ext-source"):
    ...
    verified = registry.verify(record["authority_id"], {
...
def _reconstruction(
    authority, *, obligation_id="O", target="If H, then C.", assumptions=None,
    mapping_status="PROVED", direction_status="PROVED", exception_status="NOT_APPLICABLE",
    call_id="call-reconstructor", lemmas=None,
):
    ...
    return {
        ...
        "conclusion_mapping": {
            "external_conclusion": "C", "target": target,
            "bridge_steps": ["identity"], "status": "PROVED",
        },
        "exception_analysis": {
            "excluded_cases": [], "analysis": "no exception in source statement",
            "status": exception_status,
        },
        "direction_analysis": {
            "direction": "external conclusion => target", "analysis": "same implication",
...
def _verify(registry, candidate, verdict="APPLICABLE", call_id="call-verifier"):
    return registry.verify_applicability(candidate["applicability_id"], {
        "verdict": verdict,
...
def _scheduler(tmp_path, *, dual=False, context=None):
    ...
    scheduler.add_obligation(
        "O", target_statement="If H, then C.", literature_first=True,
        dual_track=dual, context=context,
    )
    return scheduler
...
def _source_found(scheduler):
    scheduler.apply_literature_result(
        "O", verdict="EXACT_RESULT_FOUND", authority_status="VERIFIED_SOURCE_THEOREM",
    )
...
def _complete_applicability(scheduler, *, applicable=True):
    ...
    scheduler.complete_task(verifier["task_id"], {
        ...
        "authority_status": "APPLICABLE_EXTERNAL_AUTHORITY" if applicable else "APPLICABILITY_REJECTED",
        "applicability_status": "APPLICABLE_EXTERNAL_AUTHORITY" if applicable else "APPLICABILITY_REJECTED",
        "applicability_id": "app-test",
        "assumption_snapshot_hash": snapshot_hash,
...
def test_applicability_hypothesis_mismatch(tmp_path):
    test_applicability_rejects_semantic_mismatch(tmp_path, "mapping_status", "FAILED", "HYPOTHESIS_MISMATCH")


def test_applicability_wrong_direction(tmp_path):
    test_applicability_rejects_semantic_mismatch(tmp_path, "direction_status", "FAILED", "WRONG_DIRECTION")


def test_applicability_exception_mismatch(tmp_path):
    test_applicability_rejects_semantic_mismatch(tmp_path, "exception_status", "FAILED", "EXCEPTION_MISMATCH")
...
def test_same_theorem_different_obligation_has_separate_applicability(tmp_path):
    registry, authority = _verified_source(tmp_path)
    first = registry.register_applicability_reconstruction(_reconstruction(authority, obligation_id="A"))
    second = registry.register_applicability_reconstruction(_reconstruction(authority, obligation_id="B"))
...
def test_assumption_snapshot_change_invalidates_applicability(tmp_path):
    ...
    with pytest.raises(ProjectError, match="not applicable"):
        registry.require_applicable(
            authority["authority_id"], "O", "If H, then C.",
