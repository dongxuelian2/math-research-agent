...
class FakeProcess:
    ...
    def __init__(self, argv, *, stdout=SUCCESS_JSONL, stderr="", returncode=0,
                 final_text="CODEX_CLI_PROVIDER_OK", timeout=False):
        self.argv = list(argv)
        self.stdout = stdout
        self.stderr = stderr
...
def test_path_npm_cli_wins_after_windowsapps_candidate(tmp_path):
    ...
    environment["PATH"] = os.pathsep.join(
        (str(packaged_dir), environment["PATH"])
    )
...
def test_windowsapps_executable_is_rejected_before_process_start(tmp_path):
    packaged = (
        tmp_path / "WindowsApps" / "OpenAI.Codex_test" / "app" / "resources"
        / "codex.exe"
    )
...
def test_output_schema_and_chat_role_serialization(tmp_path):
    ...
    response = client.call(
        "return JSON", "system", json_schema={"type": "object"},
    )
    argv = fake.calls[0][0]
...
def test_structured_process_failures(tmp_path, stderr, error_type, retryable):
    ...
    assert secret not in archive_path.with_suffix(".raw.json").read_text(encoding="utf-8") \
        if archive_path.with_suffix(".raw.json").exists() else True
...
def test_codex_factory_does_not_construct_openai_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "openprover.math_research.providers.OpenAIResponsesClient", fail_openai,
    )
...
def test_provider_smoke_uses_exactly_one_codex_process_and_no_api(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "version",
        lambda _name: (_ for _ in ()).throw(AssertionError("OpenAI SDK not queried")),
    )
    result = cli.dispatch(Namespace(
