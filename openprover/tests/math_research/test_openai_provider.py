...
from openprover.llm._base import is_transient_error
from openprover.math_research.openai_provider import (
    OpenAIProviderError,
...
def test_text_call_preserves_roles_reasoning_and_usage(tmp_path):
    ...
    response = client.call(
        "user text", "system text", label="planner", archive_path=archive_path,
    )
...
def test_streaming_and_tool_call_parsing(tmp_path):
    ...
    response = make_client(tmp_path, fake).call(
        "u", "s", stream_callback=lambda text, kind: chunks.append((text, kind)),
    )
